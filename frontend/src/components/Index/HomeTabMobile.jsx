...
const HomeTabMobile = ({ activeTab, tabsListMobile, isFancyTabs = true, isVertical, isRightToLeftLanguage }) => {
  ...
  useEffect(() => {
    if (tabsListMobile.find((el) => el.tabUrl)) {
      setIsUrlTabs(true);
    } else {
      if (activeTab !== 0 && activeTab && activeTab < tabsListMobile.length) {
        setCurrentActiveTab(activeTab);
      } else {
        setCurrentActiveTab(0);
      }
    }
  }, [activeTab, tabsListMobile]);

  useEffect(() => {
    if (isUrlTabs) {
      const currentActiveTabUrlIndex = tabsListMobile.findIndex((el) => el.tabUrl === window.location.pathname);
  ...
  }, [isUrlTabs, tabsListMobile, currentActiveTab]);
      ...
      <div className="tabs-content-wrapper" style={{ borderRadius: isFancyTabs ? 10 : 0 }}>
        {tabsListMobile.map((el, i) => (
          ...
          >
            ...
            {currentActiveTab === i && <img src={el.content1} alt="" className='img-fluid' /> }
          </div>
        ))}
      </div>
    </div>
